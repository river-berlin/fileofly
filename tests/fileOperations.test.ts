...
describe('File Operations Tests', () => {
    ...
    test('should handle invalid line numbers', async () => {
        ...
        await expect(executeFileModification(modification)).rejects.toThrow('Invalid line numbers');
    });
    ...
    test('should handle start line greater than end line', async () => {
        ...
        await expect(executeFileModification(modification)).rejects.toThrow(
            'Start line (4) must be <= end line (2)',
        );
    });
    ...
    test('should handle create operation with invalid start line', async () => {
        ...
        await expect(executeFileModification(modification)).rejects.toThrow(
            'Create operation must start at line 1',
        );
    });
});

 ...
 * ```
 */
export async function executeFileModification(modification: FileModification): Promise<void> {
    switch (modification.operation) {
        case 'create': {
            if (modification.startLine !== 1) {
                throw new Error(
                    `Create operation must start at line 1 (received: ${modification.startLine})`,
                );
            }
        ...
        case 'update': {
            ...
            const newContent = [...beforeSection, modification.content, ...afterSection].join('\n');
